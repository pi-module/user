...
class CredentialVerify extends AbstractValidator
{
    ...
    public function isValid($value, $context = null)
    {
        ...
        $userRow = Pi::model($this->tableName, $this->module)->find($identity, 'identity');
        if (!$userRow) {
            $this->error(static::INVALID);
            return false;
        }

        $credential = Pi::service('api')->user->transformCredential($credential, $userRow->salt);
        if ($credential != $userRow->credential) {
            $this->error(static::INVALID);
            return false;
        }
